...
/**
 * The ModuleWebpackChildCompiler is a helper to allow resusing one childCompiler
 * for multile ModuleWebpackPlugin instances to improve the compilation performance.
 */
class ModuleWebpackChildCompiler {
  constructor() {
    /**
...
function extractHelperFilesFromCompilation(mainCompilation, childCompilation, filename, childEntryChunks) {
  const helperAssetNames = childEntryChunks.map((entryChunk, index) => mainCompilation.mainTemplate.hooks.assetPath.call(filename, {
    hash: childCompilation.hash,
    chunk: entryChunk,
...
/**
 * @type {WeakMap<WebpackCompiler, ModuleWebpackChildCompiler>}}
 */
...
function getChildCompiler(mainCompiler) {
  ...
  const newCompiler = new ModuleWebpackChildCompiler();
...
/**
 * @type {WeakMap<WebpackCompilation, WeakMap<ModuleWebpackChildCompiler, boolean>>}}
 */
...
function hasOutDatedTemplateCache(mainCompilation) {
  const childCompiler = getChildCompiler(mainCompilation.compiler);
  /**
   * @type {WeakMap<ModuleWebpackChildCompiler, boolean>|undefined}
   */
 ...
 *
 * @param {WebpackCompilation} mainCompilation
 * @param {ModuleWebpackChildCompiler} childCompiler
 * @returns {boolean}
 */
